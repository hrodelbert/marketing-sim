...
class Pop:
    ...
    def __init__(self, rows_cols=100):
        ...
        axes[0].imshow(self.data[-1], interpolation='nearest', cmap=cmap)
        axes[0].set_title = 'Population coverage'
        axes[1].plot(range(len(self.data)), [y.sum() for y in self.data])
...
budget = st.number_input('Budget', min_value = 1, max_value = 100, value = 10, step = 1)
if st.button("Reset"):
    st.session_state['pop'] = Pop()
if st.button("Inject"):
    st.session_state['pop'].inject()
